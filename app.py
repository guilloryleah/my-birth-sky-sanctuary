...
def format_degree(degree):
    """Return e.g. '27°' — whole degrees within the sign (0–29)."""
    return f"{int(degree % 30)}°"
...
if st.button("Unveil My Remedy"):
    if not city or not country:
        ...
        if not location:
            ...
            # --- ASCENDANT ---
            # Use whole sign houses (b"W") to match Astro.com Lahiri sidereal method.
            # ascmc[0] is the tropical ascendant degree; subtract ayanamsa for sidereal.
            cusps, ascmc = swe.houses(jd, location.latitude, location.longitude, b"W")
            asc_deg = (ascmc[0] - ayan) % 360
            asc_nak = get_nakshatra(asc_deg)
            asc_sign = get_sidereal_sign(asc_deg)
            asc_deg_fmt = format_degree(asc_deg)
            ...
            ketu_deg = (rahu_res[0] + 180) % 360
            ketu_nak = get_nakshatra(ketu_deg)
            ketu_sign = get_sidereal_sign(ketu_deg)
            ketu_deg_fmt = format_degree(ketu_deg)
